...
function sceneSetup() {
    ...
    light1.shadow.camera.left = -1;
    light1.shadow.camera.right = 1;
    ...
    light2.shadow.camera.left = -1;
    light2.shadow.camera.right = 1;
    light2.shadow.camera.top = 1;
...
img.onload = function () {
    ...
    const geometry = new ParametricGeometry((u, v, target) => {
        ...
        const y = Math.round(v * (height - 1));
        const h = heightFactor * data[(y * width + x) * 4];
        target.set(u, h, v);
    }, segments, segments);
    geometry.center();
