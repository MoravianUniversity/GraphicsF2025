...
import parquet_jpg from '../assets/textures/wood/floor-parquet-pattern-172292.jpg'
import antique_jpg from '../assets/textures/wood/abstract-antique-backdrop-164005.jpg'
import marble_jpg from '../assets/textures/marble/marble_0008_color_2k.jpg'
import ground_jpg from '../assets/textures/ground/ground_0036_color_1k.jpg'

const envMaps = (function () {
  const reflectionCube2 = new THREE.TextureLoader().load(equirectangular, (loaded) => {
    loaded.mapping = THREE.EquirectangularReflectionMapping
  })
  const refractionCube2 = new THREE.TextureLoader().load(equirectangular, (loaded) => {
...
const diffuseMaps = (function () {
  const parquet = textureLoader.load(parquet_jpg)
  parquet.encoding = THREE.SRGBColorSpace
  parquet.wrapS = THREE.RepeatWrapping
  parquet.wrapT = THREE.RepeatWrapping
  parquet.repeat.set(9, 1)

  const antique = textureLoader.load(antique_jpg)
  antique.encoding = THREE.SRGBColorSpace
  antique.wrapS = THREE.RepeatWrapping
  antique.wrapT = THREE.RepeatWrapping
  antique.repeat.set(9, 1)

  const marble = textureLoader.load(marble_jpg)
  marble.encoding = THREE.SRGBColorSpace
  marble.wrapS = THREE.RepeatWrapping
  marble.wrapT = THREE.RepeatWrapping

  const ground = textureLoader.load(ground_jpg)
  ground.encoding = THREE.SRGBColorSpace
...
const envMapKeys = Object.keys(envMaps)
const diffuseMapKeys = Object.keys(diffuseMaps)
...
const initializeGuiMaterial = (gui, mesh, material) => {
  ...
  // Probably too complex to enable
...
export const initializeGuiMeshDepthMaterial = (gui, mesh, material) => {
  const folder = gui.addFolder('THREE.MeshDepthMaterial')
  folder.add(material, 'wireframe')
}

export const initializeGuiMeshNormalMaterial = (gui, mesh, material, scene) => {
  const props = {
    vertexHelpers: false
  }
