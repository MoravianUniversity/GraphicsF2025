...
function sceneSetup() {
    ...
    const gui = new GUI();
    const actions = {
        add1000: () => { createCubes(group, materials[newCubeParams.material], 1000, newCubeParams); actions.count += 1000; },
        add100: () => { createCubes(group, materials[newCubeParams.material], 100, newCubeParams); actions.count += 100; },
        add10: () => { createCubes(group, materials[newCubeParams.material], 10, newCubeParams); actions.count += 10; },
        add1: () => { group.add(createCube(materials[newCubeParams.material]), newCubeParams); actions.count += 1; },
        clear: () => {
            while (group.children.length > 0) {
                group.children.pop().geometry.dispose();
            }
            actions.count = 0;
        },
        count: 0,
    ...
    newCubeFolder.add(newCubeParams, 'material', Object.keys(materials).filter((x) => x !== "no override"));
    ...
    groupControlFolder.add(groupParams, 'scaleX', 0.1, 5).onChange((value) => { group.scale.x = value; });
    groupControlFolder.add(groupParams, 'scaleY', 0.1, 5).onChange((value) => { group.scale.y = value; });
    groupControlFolder.add(groupParams, 'scaleZ', 0.1, 5).onChange((value) => { group.scale.z = value; });
    groupControlFolder.add(groupParams, 'rotateX', 0, Math.PI * 2).onChange((value) => { group.rotation.x = value; });
    groupControlFolder.add(groupParams, 'rotateY', 0, Math.PI * 2).onChange((value) => { group.rotation.y = value; });
    groupControlFolder.add(groupParams, 'rotateZ', 0, Math.PI * 2).onChange((value) => { group.rotation.z = value; });
...
function createCubes(group, mat, n, params) {
    for (let i = 0; i < n; i++) {
        group.add(createCube(mat, params));
    }
}
...
function createCube(mat, params) {
    ...
    mesh.rotation.set(params.rotateX, params.rotateY, params.rotateZ);
    return mesh;
}
