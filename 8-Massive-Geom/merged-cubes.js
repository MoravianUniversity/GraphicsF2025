...
function sceneSetup() {
    ...
    const gui = new GUI();
    const actions = {
        add1000000: () => { createCubes(mesh, 100000, newCubeParams); actions.count += 1000000; },
        add100000: () => { createCubes(mesh, 10000, newCubeParams); actions.count += 100000; },
        add10000: () => { createCubes(mesh, 10000, newCubeParams); actions.count += 10000; },
        add1000: () => { createCubes(mesh, 1000, newCubeParams); actions.count += 1000; },
        add100: () => { createCubes(mesh, 100, newCubeParams); actions.count += 100; },
        add10: () => { createCubes(mesh, 10, newCubeParams); actions.count += 10; },
        add1: () => { createCube(mesh, newCubeParams); actions.count += 1; },
        clear: () => {
            mesh.count = 0;
            actions.count = 0;
        },
...
function createCubes(mesh, n, params) {
    // for efficiency, this cannot use createCube() but must reimplement.
    const geoms = mesh.geometry.index ? [mesh.geometry] : [];
    for (let i = 0; i < n; i++) {
        geoms.push(createCubeGeom(params));
    }
    const old = mesh.geometry;
    mesh.geometry = BufferGeometryUtils.mergeGeometries(geoms);
    old.dispose();
}

function createCube(mesh, params) {
    const geom = createCubeGeom(params);
    const old = mesh.geometry;
    if (mesh.geometry.index) {
        mesh.geometry = BufferGeometryUtils.mergeGeometries([old, geom]);
    } else {
        mesh.geometry = geom;
    }
    old.dispose();
...
function createCubeGeom(params) {
    const range = 20;
    ...
    geom.scale(params.scaleX, params.scaleY, params.scaleZ);
    geom.rotateX(params.rotateX);
    geom.rotateY(params.rotateY);
    geom.rotateZ(params.rotateZ);
    return geom;
}